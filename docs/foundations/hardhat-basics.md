...
この章では、本サイトで使うブロックチェーン開発環境 Hardhat を説明します。  
本番用ではなく、スマートコントラクトの実行・検証を手元で試すための環境です。
...
失敗してもやり直しやすく、同じ手順を何度でも再現できることが学習環境に求められる条件である。Hardhat はこの点で扱いやすく、ブロックチェーンの初学者でも試行錯誤しやすい。
...
Hardhat 自体を深く学ぶのではなく、IW3IP の実験基盤として使います。
...
学習用のローカルチェーンと実運用ネットワークを混同しないために、この違いを把握しておいてください。
