...
ブロックチェーンは名前だけが先に知られやすく、何に向いていて、何に向いていないかが曖昧なまま理解されることが少なくありません。  
この章では、まず一般的な仕組みを整理し、そのうえで IW3IP の中でどの部分を担うのかを確認します。
...
ブロックチェーンは万能ではありません。  
何を載せるべきか、どこまでを別の仕組みに任せるべきかを切り分けて考えることが重要です。
...
このサイトのサンプルでは、まず「何を記録し、何を記録しないか」の考え方を理解することを重視しています。
...
ここを曖昧にすると、ブロックチェーンを使う理由も、使わない理由も見えにくくなります。  
IW3IP では、検証や証跡に向く情報と、容量や更新頻度の大きい情報を分けて扱います。
