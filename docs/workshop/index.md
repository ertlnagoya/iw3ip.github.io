...
講師・TA が **ワークショップ全体の説明順序と、参加者を各ハンズオンへ案内する流れ** を決めるための章です。
...
PC 構成をそろえたい授業では、`Home Assistant Demo Simulator` を Phase 1 / Phase 2 の共通導線にすると進行が安定します。
...
時間が限られている場合や、最初に全体像を見せたい場合は `assistant-demo` が最短です。
