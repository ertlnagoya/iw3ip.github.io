...
学習用としては分かりやすい一方、自然言語の表現ゆれに弱いという課題があります。
...
**LLM は計画を担うが、制御の実行は担わない** という設計です。
...
- LLM を万能な生成器としてではなく、制約付きコンポーネントとして扱える
- AI を入れるときに、どこを deterministic に残すべきか理解できる
- Phase 3 を、将来の Phase 4 的な agent 化へ拙速に進めず、堅実に拡張する考え方を学べる
