...
| **S3** OWNER tier audit log | DataUserVC owner profile | ⏳ 未検証 | OWNER で原映像取得 → `/audit/logs` に `semantic/render` 行が残る |
| **S4** Vision analyzer 顔検出 | iPhone 実写画像 | ⏳ 未検証 | `SEMANTIC_ANALYZER_BACKEND=vision` で顔ブラー bbox が SIR に含まれる |
