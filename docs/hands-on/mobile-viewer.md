...
このページは、Phase 1 で共有された結果をスマホから確認するための接続手順ページです。短時間なら URL を開いて一覧を確認するだけでも構いませんが、ハンズオンとしては「同じ LAN 上で動くこと」と「ウォレットを使う場合の入口」が分かるところまで進めるのが望ましいです。
...
ここまでで、スマホがアクセスするための公開先 URL を組み立てられます。次は実際にスマホでページを開いて表示を確認します。
...
ここまでで、閲覧用 UI への接続経路は確認できています。次は、必要なら購入操作を試し、接続できない場合の代表的な原因を整理します。
