...
「データを送れたか」だけでなく「どの条件で送られたか」を監査ログから読む点が重要です。
...
Home Assistant → publisher の基本共有経路が動いていることを確認できました。次は Consent VC による拒否ケースを見ます。
...
同じ MQTT 経路でも常に共有されるわけではないことが確認できます。Phase 3 では、共有されたイベントを assistant へ渡し、要求理解と実行を見ていきます。
