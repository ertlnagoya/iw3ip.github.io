...
このサンプルは、Home Assistant を使って IW3IP の基本経路を理解するための入口です。将来的に対象範囲を広げる場合は、次のような外部環境と接続する方向があります。

### FIWARE

FIWARE は、都市や施設、複数組織にまたがる IoT データを `Context Broker` と `IoT Agent` で扱うための基盤です。Home Assistant より構成は重くなりますが、複数主体のデータ連携や通知購読を含む研究拡張には向いています。
...
Eclipse Ditto は、機器状態やコマンドをデジタルツインとして扱うための基盤です。IW3IP の Phase 3 と組み合わせると、共有したイベントをもとに判断し、その後の状態更新や制御結果まで追いやすくなります。
...
CARLA は、都市空間、車両、歩行者、仮想センサを含むシミュレータです。Home Assistant より導入は重いですが、地域安全、防災、交通系の高度なイベントを作り、Phase 2 や Phase 3 の評価環境として使う余地があります。
