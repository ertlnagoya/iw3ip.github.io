...
> **やること**: HUSKYLENS2 で人物検知 → イベント生成 → 出品確認
>
...
Phase 1 のデバイス接続ページです。最初から実機で進める必要はなく、まず `mock` で後段パイプラインを確認してから `serial` に切り替える方が切り分けが楽です。
...
mock でパイプラインが動くことを確認できたら、次は実機の serial 入力を試します。
