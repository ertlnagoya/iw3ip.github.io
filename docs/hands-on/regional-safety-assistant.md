...
Phase 3 の考え方を理解するための代表ページです。短時間なら `plan` と `execute` だけで十分。Phase 2 との違いやモジュール境界まで理解したければ最後の整理まで読んでください。
...
`plan` と `execute` が分かれている点、判定前に必要なイベントをまとめて取得する点に注目してください。
...
自然言語要求から構造化 `plan` が作られることを確認できました。次に、その `plan` をもとにイベント評価と機器操作がどう進むかを見ます。
...
Phase 3 はイベント共有にとどまらず、要求に応じた計画・判定・実行をまとめて扱う段階です。最後に Phase 2 との違いを整理します。
